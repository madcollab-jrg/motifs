...
## Project motifs
```{r project-sets, echo=FALSE}
# Extracted analysis from https://www.tidytextmining.com/index.html 
...
project_motifs_viz <- ggplot(project_motifs, aes(n/total, fill = level)) +
...
project_tf_idf <- project_tf_idf %>%  
  mutate_at(5:7, round, 5)

library(forcats)

project_tf_idf.viz <- project_tf_idf %>%
  group_by(level) %>%
...
user_summary_sets_viz <- ggplot(subset(user_summary_sets,distinct_set>2), aes(x=log(total_sets), y=distinct_set)) +
  geom_point() + 
